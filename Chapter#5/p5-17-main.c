...
int cmp(char *s1, char *s2){
    ...
    i=j=pos1;
    if (pos2 > pos1) end = pos2;
    else end = strlen(s1) > strlen(s2) ? strlen(s2) : strlen(s1);
    
    do {
        if (dir){
            for(; s1[i] != ' ' && !isalnum(s1[i]) && s1[i] != '\0' && i < end; i++)
            for(; s2[j] != ' ' && !isalnum(s2[j]) && s1[j] != '\0' && j < end; j++)
        }
        a = fold ? tolower(*s1) : *s1;
        b = fold ? tolower(*s2) : *s2;
        s1++;
        s2++;
    } while (a == b && j < end && j < end);
